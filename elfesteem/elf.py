...
class Shdr(CStructWithStrTable):
    ...
    def name_prop(self):
        ...
        if self.flags & SHF_GROUP:
            # Include after the section name the same additional information
            # that is found in the asm generated by gcc
            name += ',"'
            if self.flags & SHF_ALLOC: name += 'a'
            if self.flags & SHF_EXECINSTR: name += 'x'
            name += 'G"'
            if self.type & SHT_PROGBITS: name += ',@progbits'
            idx = self._parent.parent.shlist.index(self._parent)
            for s in self._parent.parent: # SHList
                if s.sh.type != SHT_GROUP: continue
                if not idx in s.sections: continue
